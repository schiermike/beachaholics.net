...
/*
 ...
 modifications to fit beachaholics system
 * removed user authentication
 * taylored to mysql usage
 */


require_once("wikidownloads.php");

#-- the location of your ewiki-wrapper script
define("EWIKI_SCRIPT", "?id=");			# relative to docroot

#-- change to your needs (site lang)
define("EWIKI_NAME", "Beachaholics Wiki");		# Wiki title
define("EWIKI_PAGE_INDEX", "Hauptseite");	# default page
define("EWIKI_PAGE_LIST", "Seiten-Index");
define("EWIKI_PAGE_SEARCH", "SeiteSuchen");
define("EWIKI_PAGE_NEWEST", "NeuesteSeiten");
define("EWIKI_PAGE_HITS", "AmMeistenBesuchteSeiten");
define("EWIKI_PAGE_VERSIONS", "AmMeistenGeänderteSeiten");
define("EWIKI_PAGE_UPDATES", "ZuletztGeänderteSeiten");	# like RecentChanges
...
define("EWIKI_CHARS_U", "A-Z0-9");
define("EWIKI_CHARS", EWIKI_CHARS_L.EWIKI_CHARS_U);

#-- database
@define("EWIKI_DB_TABLE_NAME", "Wiki");	# MySQL / ADOdb
...
define("EWIKI_IMAGE_MAXSIZE", 20 * 1024 * 1024);
...
define("EWIKI_LOGLEVEL", 3);		# 0=error 1=warn 2=info 3=debug
...
$ewiki_plugins["edit_form_final"][] = "ewiki_page_edit_form_final_upload";
...
$ewiki_t["de"] = (array)@$ewiki_t["de"] + array(
	   ...
	   "PREVIEW" => "Vorschau",
	   "SAVE" => "Speichern",
	   "CANCEL_EDIT" => "Änderung verwerfen",
	   "UPLOAD_PICTURE_BUTTON" => "Bild hochladen &gt;&gt;&gt;",
	   "EDIT_FORM_1" => "Bei Problemen helfen Dir vielleicht die <a href=\"".EWIKI_SCRIPT."FormatierungsRegeln\">FormatierungsRegeln</a>.<br />",
	   "EDIT_FORM_2" => "<br />",
);
...
function ewiki_split_title ($id='', $split=-1, $entities=1) {
	...
	return($entities ? HP::toHtml($id) : $id);
}
...
function ewiki_page_links($id, &$data, $action) {
	...
	if ($pages = ewiki_get_backlinks($id)) {
		...
		$o .= ewiki_t("Keine Verweise führen zu dieser Seite.");
	}
	return($o);
}
	...
	$show = array(
      ...
      for ($v=$v_start,$first=1; ($v>=$v_end); $v--,$first=0) {
      	...
      	foreach($show as $i) {
      		...
      		{
      			$i = "Author";
      			$value = getNickForSpielerID($value);
      		}
...
function ewiki_data_update(&$data) {
	ewiki_db::UPDATE($data);
}


function ewiki_new_data($id, $flags=EWIKI_DB_F_TEXT) {
	return(ewiki_db::CREATE($id, $flags));
}
...
function ewiki_page_edit_form_final_imgupload(&$o, &$id, &$data, &$action) {
	if (EWIKI_SCRIPT_BINARY && EWIKI_UP_UPLOAD && EWIKI_IMAGE_MAXSIZE) {
		...
		. '<input type="hidden" name="'.EWIKI_UP_PARENTID.'" value="'.HP::toHtml($id).'">'
		. '&nbsp;&nbsp;&nbsp;'
...
function ewiki_page_edit_form_final_upload(&$o, &$id, &$data, &$action)
{
	$o .=ewiki_page_fileupload($id, $data, $action);
}
...
function ewiki_control_links($id, &$data, $action, $hide_hr=0, $hide_mtime=0)
{
	...
	$o .= '<br/><small>' . strftime(ewiki_t("LASTCHANGED"), @$data["lastmodified"]) . ' von '.getNickForSpielerID(@$data["SpielerID"]).'</small>';
...
function ewiki_control_links_list($id, &$data, $action_links, $version=0) {
	...
	if ($action_links)
	{
		...
		{
			$o .= $ins[1] . '<a style="font-size: 7pt;" href="' .
			( strpos($action, "://")
		...
		$o .= $ins[1] . '<a style="font-size: 7pt;" href="'.ewiki_script("SeiteSuchen").'">Seiten suchen</a>' . $ins[2] . " | ";
		$o .= $ins[1] . '<a style="font-size: 7pt;"href="'.ewiki_script("NeuesteSeiten").'">neueste Seiten</a>' . $ins[2] . " | ";
		$o .= $ins[1] . '<a style="font-size: 7pt;" href="'.ewiki_script("ZuletztGeänderteSeiten").'">zuletzt geänderte Seiten</a>' . $ins[2] . " | ";
		$o .= $ins[1] . '<a style="font-size: 7pt;" href="'.ewiki_script("DownloadSeite").'">Downloads</a>' . $ins[2] . " | ";
		$o .= $ins[1] . '<a style="font-size: 7pt;" href="'.ewiki_script("UploadSeite").'">Uploads</a>' . $ins[2];
	}
		...
		if ($iii[$in][1] & 0x0001) {
			...
			if ($lines) foreach ($lines as $s["line_i"]=>$line) {
				...
				if ($s["bmarkup"]) {
					...
					#-- html comment
					if (!strncmp($line, "&lt;!--", 7)) {
						$out .= "<!-- " . HP::toHtml(str_replace("--", "__", substr($line, 7))) . " -->\n";
						continue;
					}
...
function ewiki_link_img($href, $id, $title, $meta, $spaces, $obj, $states) {
	...
	. ' src="' . $href . '"'
	. ' alt="' . HP::toHtml($title) . '"'
	. (@$states["titled"] ? ' title="' . HP::toHtml($title) . '"' : '')
	. $size . $align
	. ($obj ? "></embed>" : " />");
	# htmlentities($title)
}
...
function ewiki_link($pagename, $title="") {
	...
	return("<a href=\"$url\">".HP::toHtml($title)."</a>");
}
...
function ewiki_binary_save_image($filename, $id="", $return=0, $add_meta=array(), $accept_all=EWIKI_ACCEPT_BINARY)
{
	global $ewiki_plugins;
	...
	$mime_types = array("application/octet-stream", "image/gif", "image/jpeg", "image/png", "application/x-shockwave-flash" );
	$ext_types = array("bin", "gif", "jpeg", "png", "swf");
...
class ewiki_db {
	...
	#
	function CREATE($id, $flags=EWIKI_DB_F_TEXT) {
		$data = array(
...
class ewiki_dbquery_result {
	...
	function get($all=0, $flags=0x0000, $type=0)
	{
		...
		do {
			...
			if (($flag_hide && ($row["flags"] & (EWIKI_DB_F_HIDDEN|EWIKI_DB_F_DISABLED)))
			|| ($type) && (($row["flags"] & EWIKI_DB_F_TYPE) != $type)) {
				$row = array();
			}
		} while (empty($row) && $flag_hide);
...
class ewiki_database_mysql {
	...
	function GET($id, $version=false) {
		...
		$result = getDB()->query("SELECT *, pagename as id FROM {$this->table}
	...
	function HIT($id) {
		$id = mysql_escape_string($id);
		getDB()->query("UPDATE {$this->table} SET hits=(hits+1) WHERE pagename='$id'");
	}
	...
	function WRITE($hash, $overwrite=0) {
		...
		$query = "$COMMAND INTO {$this->table} ($sql1) VALUES ($sql2)";
	...
	function FIND($list) {
		...
		$result = getDB()->query("SELECT pagename AS id, meta, flags FROM {$this->table} WHERE $sql");
		if ($result) {
	...
	function GETALL($fields, $mask=0, $filter=0) {
		...
		$result = getDB()->query("SELECT pagename AS id, $fields FROM
	...
	function DELETE($id, $version) {
		$id = mysql_escape_string($id);
		getDB()->query("DELETE FROM {$this->table} WHERE pagename='$id' AND version=$version");
	}
